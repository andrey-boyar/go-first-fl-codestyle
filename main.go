...
func defence(char_name, char_class string) string {
	switch char_class {
	case "warrior":
		fmt.Sprintf("%s блокировал %d урона.", char_name, 10+randint(5, 10))
	case "mage":
		fmt.Sprintf("%s блокировал %d урона.", char_name, 10+randint(-2, 2))
	case "healer":
		fmt.Sprintf("%s блокировал %d урона.", char_name, 10+randint(2, 5))
	//default:
